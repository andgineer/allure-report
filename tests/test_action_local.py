...
def test_create_directories(env, expected_index_file):
    ...
    with patch("subprocess.run") as mock_run:
        ...
        assert allure_gen.allure_report.exists()
        assert (allure_gen.allure_results / "executor.json").exists()
        assert (allure_gen.allure_results / "history").exists()
